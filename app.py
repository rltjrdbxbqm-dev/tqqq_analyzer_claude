    ...
    if data is not None:
        ...
        with st.expander("📊 포지션 계산 과정 상세"):
            ...
            with col1:
                st.markdown(f"""
                **오차율 매수 전략:**
                - 활성 신호: {results['active_error_count']}개
                - 조정 비중: **+{today_port['error_adjustment']:.1%}** (GLD → TQQQ)
                """)
                
                if results['today_portfolio']['active_error_strategies']:
                    st.markdown("**활성화된 전략:**")
                    for strategy in results['today_portfolio']['active_error_strategies']:
                        st.markdown(f"- {strategy['name']}: 오차율 {strategy['deviation']:.1f}% ≤ {strategy['threshold']}%")
            
            with col2:
                st.markdown(f"""
                **최적화 매도 전략:**
                - 매도 신호: {today_port['sell_strategies_count']}개
                - 조정 비중: **{today_port['optimized_adjustment']:.1%}** (TQQQ → 현금)
